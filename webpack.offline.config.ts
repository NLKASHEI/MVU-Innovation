...
function config(_env: any, _argv: any): webpack.Configuration {
    // 获取构建时常量
    const buildDate = (() => {
        const date = new Date();
    ...
    return {
        ...
        // 离线单文件: 不生成 sourcemap, 保持单文件
        devtool: false,
        watchOptions: {
            ignored: ['**/dist', '**/node_modules'],
        },
        entry: path.join(import.meta.dirname, 'src/offline_entry.ts'),
        target: 'browserslist',
        output: {
            ...
            // 离线单文件: 输出到 ../MVU-Offline
            filename: `mvu_bundle_full.js`,
            path: path.join(import.meta.dirname, '..', '..', 'MVU-Offline'),
            chunkFilename: `mvu_bundle_full.[contenthash].chunk.js`,
            asyncChunks: true,
            clean: true,
            publicPath: '',
            library: {
        ...
        plugins: [
            ...
            new webpack.DefinePlugin({
                ...
                __COMMIT_ID__: JSON.stringify('offline'),
        ...
        optimization: {
            ...
            minimizer: [
                new TerserPlugin({
                    // 离线单文件: license 注释内联, 不抽取到单独的 .LICENSE.txt
        ...
        // 离线单文件: 只把酒馆环境提供的全局库外部化, 其余依赖(vue / klona /
        // compare-versions / json5 / jsonrepair / mathjs / pinia 等)全部本地内联,
        // 避免运行期依赖 CDN。
