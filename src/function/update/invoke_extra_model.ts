...
async function requestReply(generation_id?: string, batch_id?: string): Promise<string> {
    ...
    // [革新版·缓存率] 随机头部仅 Gemini 系需要（防破限被记录）；对 DeepSeek/Kimi/GLM 等
    // 隐式前缀缓存模型，随机头放 prompt 最前会让 L0 前缀每次变化、缓存率归零。
    // 因此：非 gemini 一律不生成；gemini 保留但移到 prompt 尾部，不破坏稳定前缀。
    ...
    const result = await generateRaw({
        ...config,
        ordered_prompts: [
            // L0 稳定前缀：head 固定，persona/char/world_info 保持稳定 → 前缀缓存可命中
